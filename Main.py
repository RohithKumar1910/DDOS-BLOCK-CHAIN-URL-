...
main = Tk()
main.title("A Trusty Blockchain-based Domain Name System")
...
def saveCertificate():
    global filename
    text.delete('1.0', END)
    filename = askopenfilename(initialdir = "URLCertificate_templates")
    ...
    if len(roll_no) > 0 and len(name) > 0 and len(contact) > 0:
        ...
        text.insert(END,"Domain Name Digital Signature : "+str(digital_signature)+"\n\n")
    ...
    for i in range(len(blockchain.chain)):
        if i > 0:
            ...
            if arr[3] == digital_signature:
                text.insert(END,"Uploaded Domain Certificate Validation Successfull\n")
                text.insert(END,"Domain User Details extracted from Blockchain after Domain Validation\n\n")
                text.insert(END,"Domain Name : "+arr[0]+"\n")
                text.insert(END,"Owner Name : "+arr[1]+"\n")
                text.insert(END,"Contact No   : "+arr[2]+"\n")
                text.insert(END,"Digital Sign : "+arr[3]+"\n")
                flag = False
                break
    if flag:
        text.insert(END,"DDOS Attack Detected or certificate modified")
    

font = ('times', 15, 'bold')
title = Label(main, text='A Trusty Blockchain-based Domain Name System')
...
l1 = Label(main, text='Domain Name :')
...
verifyButton = Button(main, text="Verify Certificate Attack", command=verifyCertificate)
